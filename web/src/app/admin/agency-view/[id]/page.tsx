...
export default function AgencyViewPage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-outline-variant/10 bg-surface px-8 py-4">
        <div className="flex items-center gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="font-headline text-[22px] font-bold tracking-tight text-on-surface">
                {agency.name}
              </h1>
        ...
        <button
          type="button"
          onClick={handleClose}
          className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-container-low text-on-surface-variant hover:bg-error/10 hover:text-error transition-colors"
          title="창 닫기"
        >
          <span
            className="material-symbols-outlined text-[22px]"
            style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 22" }}
          >
            close
          </span>
        </button>
      ...
      <div className="mx-auto max-w-[1600px] space-y-8 p-8">
            ...
            <div className="rounded-xl border border-outline-variant/15 p-4">
              ...
              {usageStats.maxDrivers > 0 && (
                <div className="flex h-4 w-full overflow-hidden rounded-full bg-surface-container-low">
                  <div
                    className={`h-full transition-all duration-500 ${usageStats.overCount > 0 ? 'bg-error' : 'bg-primary'}`}
                    style={{ width: `${Math.min(100, (usageStats.currentAppActive / usageStats.maxDrivers) * 100)}%` }}
                  />
                </div>
              )}
              ...
              <div className="mt-3 grid grid-cols-3 gap-3 text-center">
                ...
                <div className="rounded-lg bg-surface-container-low p-2">
                  ...
                  <p className="text-sm font-semibold text-error">{usageStats.overCount}명</p>
                </div>
                <div className="rounded-lg bg-surface-container-low p-2">
                  <p className="text-xs text-on-surface-variant">월 요금</p>
                  <p className="text-sm font-semibold text-on-surface">{formatKRW(agency.monthly_fee)}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-surface-container-lowest p-6 shadow-ambient">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="rounded-xl border border-outline-variant/15 p-3 text-center">
                ...
                <p className="mt-1 text-xl font-bold text-primary">{pointStats.balance.toLocaleString('ko-KR')}</p>
              </div>
              <div className="rounded-xl border border-outline-variant/15 p-3 text-center">
                <p className="text-xs text-on-surface-variant">누적 충전</p>
                <p className="mt-1 text-xl font-bold text-tertiary">{pointStats.totalCharged.toLocaleString('ko-KR')}</p>
              </div>
              <div className="rounded-xl border border-outline-variant/15 p-3 text-center">
                <p className="text-xs text-on-surface-variant">누적 사용</p>
                <p className="mt-1 text-xl font-bold text-on-surface">{pointStats.totalUsed.toLocaleString('ko-KR')}</p>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {pointStats.recentTransactions.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="py-6 text-center text-on-surface-variant">포인트 거래 내역이 없습니다.</td>
                    </tr>
                  ) : (
                    pointStats.recentTransactions.map((tx) => (
                      <tr key={tx.id} className="border-t border-outline-variant/10">
                        <td className="py-2 pr-3">
                          <Badge label={POINT_TX_LABEL[tx.type] ?? tx.type} variant={POINT_TX_VARIANT[tx.type] ?? 'default'} />
                        </td>
                        <td className={`py-2 pr-3 font-semibold ${tx.type === 'charge' || tx.type === 'bonus' || tx.type === 'refund' ? 'text-tertiary' : 'text-error'}`}>
                          {tx.type === 'charge' || tx.type === 'bonus' || tx.type === 'refund' ? '+' : '-'}
                          {Math.abs(tx.amount).toLocaleString('ko-KR')}
                        </td>
                        <td className="py-2 pr-3 text-on-surface-variant">{tx.balance_after.toLocaleString('ko-KR')}</td>
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
          ...
          <div className="rounded-2xl bg-surface-container-lowest p-6 shadow-ambient">
            <div className="mb-1 flex items-center gap-2">
              <span className="material-symbols-outlined text-[20px] text-primary" style={{ fontVariationSettings: "'FILL' 0, 'wght' 300, 'GRAD' 0, 'opsz' 20" }}>description</span>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {contractStats.recent.length === 0 ? (
                    <tr><td colSpan={4} className="py-6 text-center text-on-surface-variant">계약서가 없습니다.</td></tr>
                  ) : (
                    contractStats.recent.map((c) => (
                      <tr key={c.id} className="border-t border-outline-variant/10">
                        ...
                        <td className="py-2.5 pr-3"><Badge label={CONTRACT_STATUS_LABEL[c.status] ?? c.status} variant={CONTRACT_STATUS_VARIANT[c.status] ?? 'default'} /></td>
                        <td className="py-2.5 pr-3 text-on-surface-variant">{c.sent_at ? formatDateTime(c.sent_at) : '-'}</td>
                        <td className="py-2.5 text-on-surface-variant">{c.signed_at ? formatDateTime(c.signed_at) : '-'}</td>
                      </tr>
                    ))
                  )}
                </tbody>
          ...
          <div className="rounded-2xl bg-surface-container-lowest p-6 shadow-ambient">
            <div className="mb-1 flex items-center gap-2">
              <span className="material-symbols-outlined text-[20px] text-primary" style={{ fontVariationSettings: "'FILL' 0, 'wght' 300, 'GRAD' 0, 'opsz' 20" }}>receipt_long</span>
              ...
              <div className="mt-4 rounded-xl border border-outline-variant/15 p-3">
                ...
                <div className="mt-2 flex gap-4 text-sm">
                  ...
                  {settlementStats.jobFailed > 0 && (
                    <span className="text-error">실패 <span className="font-semibold">{settlementStats.jobFailed}</span>건</span>
                  )}
                </div>
              </div>
            )}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {settlementStats.recent.length === 0 ? (
                    <tr><td colSpan={4} className="py-6 text-center text-on-surface-variant">정산서가 없습니다.</td></tr>
                  ) : (
                    settlementStats.recent.map((s) => (
                      <tr key={s.id} className="border-t border-outline-variant/10">
                        ...
                        <td className="py-2.5 pr-3"><Badge label={SETTLEMENT_STATUS_LABEL[s.status] ?? s.status} variant={SETTLEMENT_STATUS_VARIANT[s.status] ?? 'default'} /></td>
                        <td className="py-2.5 pr-3 text-on-surface">{s.net_amount != null ? formatKRW(s.net_amount) : '-'}</td>
                        <td className="py-2.5 text-on-surface-variant">{s.sent_at ? formatDateTime(s.sent_at) : '-'}</td>
                      </tr>
                    ))
                  )}
                </tbody>
        ...
        <div className="rounded-2xl bg-surface-container-lowest p-6 shadow-ambient">
          <div className="mb-1 flex items-center gap-2">
            <span className="material-symbols-outlined text-[20px] text-primary" style={{ fontVariationSettings: "'FILL' 0, 'wght' 300, 'GRAD' 0, 'opsz' 20" }}>payments</span>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-surface-container-lowest">
                <tr className="text-left text-xs text-on-surface-variant">
                  ...
                  <th className="pb-2 pr-3">상태</th>
                  <th className="pb-2 pr-3">금액</th>
                  <th className="pb-2 pr-3">결제 수단</th>
                  <th className="pb-2">결제일</th>
                </tr>
              </thead>
              <tbody>
                {paymentStats.recent.length === 0 ? (
                  <tr><td colSpan={5} className="py-6 text-center text-on-surface-variant">결제 내역이 없습니다.</td></tr>
                ) : (
                  paymentStats.recent.map((p) => (
                    <tr key={p.id} className="border-t border-outline-variant/10">
                      ...
                      <td className="py-2.5 pr-3"><Badge label={PAYMENT_STATUS_LABEL[p.status] ?? p.status} variant={PAYMENT_STATUS_VARIANT[p.status] ?? 'default'} /></td>
                      <td className="py-2.5 pr-3 text-on-surface">{p.amount != null ? formatKRW(p.amount) : '-'}</td>
                      <td className="py-2.5 pr-3 text-on-surface-variant">{p.payment_method || '-'}</td>
                      <td className="py-2.5 text-on-surface-variant">{p.paid_at ? formatDateTime(p.paid_at) : '-'}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="rounded-2xl bg-surface-container-lowest p-6 shadow-ambient">
          <div className="mb-1 flex items-center gap-2">
            <span className="material-symbols-outlined text-[20px] text-primary" style={{ fontVariationSettings: "'FILL' 0, 'wght' 300, 'GRAD' 0, 'opsz' 20" }}>groups</span>
          ...
          <div className="flex h-3 w-full overflow-hidden rounded-full bg-surface-container-low mb-5">
            {driverStats.total > 0 && (
              <>
                <div className="h-full bg-primary transition-all duration-500" style={{ width: `${(driverStats.appActive / driverStats.total) * 100}%` }} />
                <div className="h-full bg-outline-variant/30 transition-all duration-500" style={{ width: `${((driverStats.total - driverStats.appActive) / driverStats.total) * 100}%` }} />
              </>
            )}
          </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {driverStats.drivers.length === 0 ? (
                  <tr><td colSpan={4} className="px-4 py-8 text-center text-on-surface-variant">소속 기사가 없습니다.</td></tr>
                ) : (
