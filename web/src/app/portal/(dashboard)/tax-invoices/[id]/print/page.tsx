    ...
    <div className="min-h-screen bg-white p-4 print:p-0">
      ...
      <div className="max-w-[800px] mx-auto border-2 border-gray-800">
            ...
            <table className="w-full border-collapse">
              <tbody>
                <tr>
                  ...
                  <td className={`${cellCls} font-mono tracking-wider`} colSpan={3}>
                    {supplier.regNumber}
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td className={`${headerCls} w-20`}>업태</td>
                  <td className={cellCls}>{supplier.businessType}</td>
                  <td className={`${headerCls} w-16`}>종목</td>
        ...
        <table className="w-full border-collapse border-b border-gray-800">
          <thead>
            <tr>
              ...
              <th className={headerCls}>{isWithholding ? '원천세액' : '세액'}</th>
        ...
        <table className="w-full border-collapse border-b border-gray-800">
          <thead>
            <tr>
              ...
              <th className={`${headerCls} w-24`}>비고</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className={`${cellCls} text-center`}>
                {Number(invoice.year_month.split('-')[1])}
              </td>
              <td className={cellCls}>
                {isWithholding
                  ? '운송 용역 대금(원천징수)'
                  : `${formatYearMonth(invoice.year_month)} 운송 대금`}
              </td>
              ...
              <td className={cellCls}>{invoiceTypeLabel(invoice.invoice_type)}</td>
            </tr>
            {[...Array(3)].map((_, index) => (
              <tr key={index}>
        ...
        <table className="w-full border-collapse border-b border-gray-800">
          <tbody>
            <tr>
              ...
              <td className={headerCls}>{isWithholding ? '원천세액' : '세액'}</td>
              <td className={`${cellCls} text-right font-mono font-bold`}>
                {invoice.tax_amount.toLocaleString()}
              </td>
            </tr>
        ...
        <div className="p-4 text-xs text-gray-600 space-y-2">
          <p>
            위 금액을 {isManualReverse ? '역발행' : isWithholding ? '원천징수' : '청구'} 기준으로 확인합니다.
          </p>
          <p className="text-right">{formatDate(invoice.issued_at)}</p>
          <div className="flex justify-between pt-2">
            <span>공급자 {supplier.companyName}</span>
            <span>공급받는 자 {buyer.companyName}</span>
          </div>
        </div>
      </div>
    </div>
